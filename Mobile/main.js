...
class Card {
  ...
  onTouchEnd() {
    let touchtime = new Date().getTime() - tapstart;
    if (touch.target && touchtime < 200 && touch.target.div.style.color == 'white') {
      if (touch.target.rotation < 270) {
        touch.target.rotation += 90;
        touch.target.div.style.transform = `rotate(${touch.target.rotation}deg)`;
      }
      else {
        touch.target.rotation = 0;
        touch.target.div.style.transform = `rotate(${touch.target.rotation}deg)`;
      }
    }
    touch.drag = false;
...
document.addEventListener('touchstart', (e) => {
  ...
  {
    if (!touch.drag) touch.target.div.style.transform = "rotate(90deg)";
  }
},{passive: false});
